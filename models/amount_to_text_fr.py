...
class Travel(models.Model):
    ...
    def _amount_in_words(self):
        """ transform amount ot text."""
        for rec in self:
            rec.price_to_text = amount_to_text_fr(rec.price, rec.currency_id.symbol)

    @api.depends('price', 'paid_amount')
    def _get_left_amount(self):
        """ compute left amount"""
        for rec in self:
            left_amount = rec.price - rec.paid_amount
            rec.amount_left = left_amount > 0.000000001 and left_amount or 0.00

    price_to_text = fields.Text(string=u'الثمن',
                                store=True, readonly=True, compute='_amount_in_words')

    amount_left = fields.Float(string=u'الباقي',
                               store=True, compute='_get_left_amount')